...
void
proc_kvmmap(pagetable_t proc_kpagetable,uint64 va, uint64 pa, uint64 sz, int perm){
...
// transfer vm to kvm
// by direct address reflection.
void uvm2ukvm(pagetable_t old, pagetable_t new, uint64 st, uint64 ed){
  pte_t *pte_old, *pte_new;
  uint64 pa,i;
  uint flags;
  for(i=PGROUNDUP(st);i<ed;i+=PGSIZE){
    if((pte_old = walk(old, i, 0))==0)
      panic("uvm2ukvm: pte_old should exist");
    if((*pte_old & PTE_V) ==0)
      panic("uvm2ukvm: page not present");
    pa = PTE2PA(*pte_old);
    flags = PTE_FLAGS(*pte_old) & (~PTE_U);

    if((pte_new = walk(new, i, 1))==0)
      panic("uvm2ukvm: pte_new should exist");
